...
def apply_guardrails(
    ...
    for t in table:
        if t in GUARDRAIL_POLICIES["role_permissions"].get(role, []):
            continue
        else:
            violations.append(f"Role '{role}' is not authorized to access table '{t}'")
    ...
    if violations:
        logger.warning(
            f"Guardrail Violations Detected | Role: {role} | Query: '{query}' | Violations: {violations}"
        )
